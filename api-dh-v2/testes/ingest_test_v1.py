...
def db_insert(filename):
    ...
    if fileprefix in ('prod_inc_prsn_count', 'prod_new_prsn_count_filter', 'index_l21',
                      'obj_filter_count', 'obj_structure_count', 'un_filters', 'personas'):
...
def untar_files(localpath, fi):
    ...
    if gpg_time > ungpg_time:
        print("Decrypting {f} into {d}".format(f=fi,
                                               d=ungpg))
        ##original
        os.system('gpg --batch --passphrase-fd 1 --passphrase-file ~/.pass --output {p}{f1} {p}{f2}'.format(p=localpath,
                                                                                                            f1=ungpg,
                                                                                                            f2=fi))
        #windows
        #os.system('gpg --batch --passphrase-fd 1 --passphrase-file pass.pass --output {p}{f1} {p}{f2}'.format(p=localpath,
        #                                                                                                    f1=ungpg,
        #                                                                                                    f2=fi))
...
print('files in local:' + str(local_file_list))

sftp = connect_sftp()

for fi in sftp.listdir(config['remotepath']):
    if fi[0:4] not in ['crea', 'prod', 'inde', 'obj_', 'un_f', 'pers']:
...
#for fi in sorted([remote_file for remote_file in local_file_list], key=custom_sorter):
    if any(x in fi for x in ('create_table', 'create_personas', 'index', 'prod', 'obj', 'un', 'personas')):
